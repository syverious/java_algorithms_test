...
public class QuantumMechanics {
    ...
    private static double radiusOfSpace = 321;            //180,
    private static double wavelength = 10;    // 波长 λ = h/p = h/mv
    private static double[] centerOfObject = {160.5, 0.5, 0};    // 圆形物体中心位置
    private static double radiusOfObject = 160;         // 圆形物体半径

    private static int drawWidth = 800;
    private static int drawHeight = 642;
    private static int drawDepth = 321;
    private static int centerX = 400;
    private static double centerY = 321.5;
    private static int centerZ = 161;
        ...
        for (int x = 0; x < drawWidth; x++) {
            for (int y = 0; y < drawHeight; y++) {
                ...
                if (Math.pow(xSpace - centerOfObject[0], 2) +
                        ...
                        Math.pow(zSpace - centerOfObject[2], 2) < raduisSquare &&
                        Math.pow(dxSpace - centerOfObject[0], 2) +
                                Math.pow(dySpace - centerOfObject[1], 2) > raduisSquare) {

                    double distance = Math.sqrt(
                            Math.pow(xSpace - dxSpace, 2) +
                                    Math.pow(ySpace - dySpace, 2) +
                                    Math.pow(-zSpace, 2));
        ...
        for (int x = 0; x < drawWidth; x++) {
            for (int y = 0; y < drawHeight; y++) {
                for (int z = 0; z < drawDepth; z++) {
                    ...
                    // if (Math.abs(x - centerX) > config.radiusOfSpace[0] || Math.abs(y - centerY) > config.centerOfSpace) {
                    //     probMap[x][y] = 0;
                    //     continue;
                    // }
                    // int z =
...
//                    }
                    // 改为立方体范围
                    if (Math.abs(dx) > radiusOfSpace || Math.abs(dy) > radiusOfSpace || Math.abs(dz) > radiusOfSpace) {
                        probMap[x][y] = 0;    //epsilon
                        continue;
                    }
                    // 采用镜像翻转减少一半计算
                    if(y > centerY) {
                        probMap[x][y] = probMap[x][drawHeight - y];
                    } else {
            ...
            if (x % 10 == 0) System.out.println(x + " pix");
        }

        // 创建BufferedImage绘制灰度图
        BufferedImage image = new BufferedImage(drawWidth, drawHeight, BufferedImage.TYPE_BYTE_GRAY);
        ...
        for (int x = 0; x < drawWidth; x++) {
            for (int y = 0; y < drawHeight; y++) {
                // 归一化并转换为灰度值(0-255)
                int grayValue = (int)(255 * Math.sqrt(probMap[x][y] / maxProb));
        ...
        // 保存图像文件
        try {
            File output = new File("quantum_interference_sphere.png");
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        QuantumMechanics pathIntegral = new QuantumMechanics();
...
//                    }
                    ...
                    if (x % 10 == 0) System.out.println(finalI + ": "+ x + " pix");
                }
            });
            threads[i].start();
        }
